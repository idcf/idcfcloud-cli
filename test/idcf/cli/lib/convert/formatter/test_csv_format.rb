          ...
          class TestCsvFormat < TestBase
            ...
            def test_format_error_msg(data)
              str = <<-CSV.strip_heredoc
                ...
                "400","[""msg"",""msg2""]","[]"
              CSV
              assert_equal(@target.format(data), str.strip)
            end
            ...
            def test_format_error_msg_exist_data(data)
              str = <<-CSV.strip_heredoc
                "status","message","data"
                "400","[""msg"",""msg2""]","[1,2]"
              CSV
            ...
            def test_format_success_none(data)
              assert_equal(@target.format(data[:data]), '')
            end
            ...
            def test_format_success_flat_array(data)
              str = <<-CSV.strip_heredoc
                ...
                "40"
              CSV
              assert_equal(@target.format(data[:data]), str.strip)
            end
            ...
            def test_format_success_one_data(data)
              str = <<-CSV.strip_heredoc
                "id","txt","conf","list"
                "1-0-1","1-0-1 sample","{""id"":""1-1-1"",""txt"":""1-1 sample""}","[{""id"":""1-2-1"",""txt"":""2-1 sample""},{""id"":""1-2-2"",""txt"":""2-2 sample""}]"
              CSV
              assert_equal(@target.format(data[:data]), str.strip)
            end
            ...
            def test_format_success_multi_data(data)
              str = <<-CSV.strip_heredoc
                "id","txt","conf","list"
                "1-0-1","1-0-1 sample","{""id"":""1-1-1"",""txt"":""1-1 sample""}","[{""id"":""1-2-1"",""txt"":""2-1 sample""},{""id"":""1-2-2"",""txt"":""2-2 sample""}]"
                "2-0-1","2-0-1 sample","{""id"":""2-1-1"",""txt"":""1-1 sample""}","[{""id"":""2-2-1"",""txt"":""2-1 sample""},{""id"":""2-2-2"",""txt"":""2-2 sample""}]"
              CSV
              assert_equal(@target.format(data[:data]), str.strip)
            end
          end
